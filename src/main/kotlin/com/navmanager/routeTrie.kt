...
data class RouteNode<P>(
  ...
  fun prettyPrint(indent: String = ""): String {
    ...
    paramChild?.let { child -> sb.append(child.prettyPrint("$indent│   ")) }
    return sb.toString()
  }
}
...
class RouteTree<P> {
  ...
  fun getNode(routeSegment: String): RouteNode<P>? {
  ...
  fun insert(path: String, handler: P): RouteNode<P> {
    ...
    if (current.isEndOfRoute) {
      throw DuplicateValueException(message = "duplicate route not allowed", duplicateValue = path)
    }
